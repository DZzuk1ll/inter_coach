...
- **前端**：Next.js 16 (App Router) + TypeScript 严格模式 + Tailwind CSS 4 + shadcn/ui + TanStack Query + next-themes
...
- `backend/app/services/llm_client.py` — openai SDK 封装，`chat()`、`chat_stream()` 和 `chat_json()` 三个方法
...
- `frontend/src/app/globals.css` — 设计系统：CSS 变量色板（12 级中性灰 + 靛色强调色 + 状态色）、Light/Dark 双主题（via `.dark` class + next-themes）

### 前端设计上下文

进行任何前端设计工作前，**必须先阅读 `.impeccable.md`**（项目根目录），其中包含品牌个性、美学方向、设计原则等关键上下文。所有前端颜色使用 `globals.css` 中定义的 CSS 变量，不硬编码色值。Dark 主题通过 `.dark` class 选择器实现（next-themes 管理），不使用 `@media (prefers-color-scheme)`。

### 测试

- 后端测试使用 SQLite 内存数据库 + mock LLM（`tests/conftest.py` 已配置 JSONB→JSON 适配）
- 测试文件：`test_users.py`、`test_project_flow.py`、`test_interview_flow.py`、`test_projects.py`、`test_interviews.py`、`test_health.py`
- 面试引擎测试需 mock `knowledge_service`（避免 pgvector 依赖）和 `LLMClient`
...
- CORS 来源在 `config.yaml` 的 `app.cors_origins` 配置，部署时需设为实际域名
